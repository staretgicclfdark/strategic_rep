...
class StrategicRandomFriendsRunner:
    ...
    def get_models(self, force_train_hadart, force_train_loan_model, feature_list_to_use, use_cv):
        self.feature_list_to_use = feature_list_to_use
        self.f_hardt = get_hardt_model(self.cost_factor, real_train_f_star_loan_status_path, force_train_hadart)
    ...
    def __init__(self, number_of_friends_to_learn_list: list, cost_factor: float, epsilon: float, feature_list_to_use: list,
                 force_train_hadart: bool, force_train_svm_loan_model: bool, spare_cost: float, use_cv: bool = False):
        ...
        self.get_models(force_train_hadart, force_train_svm_loan_model, feature_list_to_use, use_cv=use_cv)
        self.get_datasets_and_f_grade()
    ...
    def run_strategic_random_exp_for_clf(self, num_friend: int, clf_name: str, train_val_model_loan_status,
        ...
        member_dict = get_member_friends_dict(num_friend, train_val_model_loan_status,
                                              list(self.test_f_star['MemberKey']), member_friend_dict_path)
    ...
    def execute_strategic_random_friends_exp(self):
        ...
        for num_friend in self.m_list:
            print(num_friend)
            friends_modify_on_svm_strategic_data, data_svm_res_dict = self.run_strategic_random_exp_for_clf(num_friend, 'SVM', self.train_val_svm_loan_status, self.f_svm, self.svm_folder)
            friends_modify_on_hardt_strategic_data, data_hardt_res_dict = self.run_strategic_random_exp_for_clf(num_friend, 'Hardt', self.train_val_hardt_loan_status, self.f_hardt, self.hardt_folder)
        ...
        result_exp.plot_social_inequality_graph(modified_data_folder, self.spare_cost)
        return result_exp
